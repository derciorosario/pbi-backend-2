// controllers/_serviceAudienceHelpers.js
...
async function validateAudienceHierarchy({ categoryIds, subcategoryIds, subsubCategoryIds }) {
  try {
    ...
    console.log('Validating service hierarchy:', { cats: cats.length, subs: subs.length, subsubs: subsubs.length });

    // Limit the number of items to prevent memory issues
    if (cats.length > 100 || subs.length > 500 || subsubs.length > 1000) {
      console.warn('Large number of service audience items detected, limiting validation');
      return {
    ...
    if (subs.length && cats.length) {
      console.log('Validating service subcategories against categories...');
      ...
      console.log('Found service subcategories:', subcats.length, 'Valid:', validSubcats.length);
    }

    if (subsubs.length && validSubs.size > 0) {
      console.log('Validating service subsubcategories against subcategories...');
      ...
      console.log('Found service subsubcategories:', subsubcats.length, 'Valid:', validSubsubcats.length);
    }
    ...
    console.log('Service hierarchy validation passed. Valid items:', {
    ...
    console.error('Error in service validateAudienceHierarchy:', error);
...
async function setServiceAudience(service, { identityIds, categoryIds, subcategoryIds, subsubCategoryIds }) {
  console.log('Original service audience data:', { identityIds, categoryIds, subcategoryIds, subsubCategoryIds });

  // Validate and fix hierarchy
  const validated = await validateAudienceHierarchy({ categoryIds, subcategoryIds, subsubCategoryIds });

  console.log('Validated service audience data:', validated);

  // Properly clear existing associations and set new ones
  if (identityIds !== undefined) {
    await service.setAudienceIdentities([]); // Clear first
    if (identityIds && identityIds.length > 0) {
      await service.setAudienceIdentities(identityIds);
    }
  }

  if (categoryIds !== undefined) {
    await service.setAudienceCategories([]); // Clear first
    if (validated.categoryIds && validated.categoryIds.length > 0) {
      await service.setAudienceCategories(validated.categoryIds);
    }
  }

  if (subcategoryIds !== undefined) {
    await service.setAudienceSubcategories([]); // Clear first
    if (validated.subcategoryIds && validated.subcategoryIds.length > 0) {
      await service.setAudienceSubcategories(validated.subcategoryIds);
    }
  }

  if (subsubCategoryIds !== undefined) {
    await service.setAudienceSubsubs([]); // Clear first
    if (validated.subsubCategoryIds && validated.subsubCategoryIds.length > 0) {
      await service.setAudienceSubsubs(validated.subsubCategoryIds);
    }
  }
}
