// controllers/_productAudienceHelpers.js
...
async function validateAudienceHierarchy({ categoryIds, subcategoryIds, subsubCategoryIds }) {
  try {
    ...
    console.log('Validating product hierarchy:', { cats: cats.length, subs: subs.length, subsubs: subsubs.length });

    // Limit the number of items to prevent memory issues
    if (cats.length > 100 || subs.length > 500 || subsubs.length > 1000) {
      console.warn('Large number of product audience items detected, limiting validation');
      return {
    ...
    if (subs.length && cats.length) {
      console.log('Validating product subcategories against categories...');
      ...
      console.log('Found product subcategories:', subcats.length, 'Valid:', validSubcats.length);
    }

    if (subsubs.length && validSubs.size > 0) {
      console.log('Validating product subsubcategories against subcategories...');
      ...
      console.log('Found product subsubcategories:', subsubcats.length, 'Valid:', validSubsubcats.length);
    }
    ...
    console.log('Product hierarchy validation passed. Valid items:', {
    ...
    console.error('Error in product validateAudienceHierarchy:', error);
...
async function setProductAudience(product, { identityIds, categoryIds, subcategoryIds, subsubCategoryIds }) {
  console.log('Original product audience data:', { identityIds, categoryIds, subcategoryIds, subsubCategoryIds });

  // Validate and fix hierarchy
  const validated = await validateAudienceHierarchy({ categoryIds, subcategoryIds, subsubCategoryIds });

  console.log('Validated product audience data:', validated);

  // Properly clear existing associations and set new ones
  if (identityIds !== undefined) {
    await product.setAudienceIdentities([]); // Clear first
    if (identityIds && identityIds.length > 0) {
      await product.setAudienceIdentities(identityIds);
    }
  }

  if (categoryIds !== undefined) {
    await product.setAudienceCategories([]); // Clear first
    if (validated.categoryIds && validated.categoryIds.length > 0) {
      await product.setAudienceCategories(validated.categoryIds);
    }
  }

  if (subcategoryIds !== undefined) {
    await product.setAudienceSubcategories([]); // Clear first
    if (validated.subcategoryIds && validated.subcategoryIds.length > 0) {
      await product.setAudienceSubcategories(validated.subcategoryIds);
    }
  }

  if (subsubCategoryIds !== undefined) {
    await product.setAudienceSubsubs([]); // Clear first
    if (validated.subsubCategoryIds && validated.subsubCategoryIds.length > 0) {
      await product.setAudienceSubsubs(validated.subsubCategoryIds);
    }
  }
}
