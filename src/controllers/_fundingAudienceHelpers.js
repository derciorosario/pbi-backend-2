// controllers/_fundingAudienceHelpers.js
...
async function validateAudienceHierarchy({ categoryIds, subcategoryIds, subsubCategoryIds }) {
  try {
    ...
    console.log('Validating funding hierarchy:', { cats: cats.length, subs: subs.length, subsubs: subsubs.length });

    // Limit the number of items to prevent memory issues
    if (cats.length > 100 || subs.length > 500 || subsubs.length > 1000) {
      console.warn('Large number of funding audience items detected, limiting validation');
      return {
    ...
    if (subs.length && cats.length) {
      console.log('Validating funding subcategories against categories...');
      ...
      console.log('Found funding subcategories:', subcats.length, 'Valid:', validSubcats.length);
    }

    if (subsubs.length && validSubs.size > 0) {
      console.log('Validating funding subsubcategories against subcategories...');
      ...
      console.log('Found funding subsubcategories:', subsubcats.length, 'Valid:', validSubsubcats.length);
    }
    ...
    console.log('Funding hierarchy validation passed. Valid items:', {
    ...
    console.error('Error in funding validateAudienceHierarchy:', error);
...
async function setFundingAudience(funding, { identityIds, categoryIds, subcategoryIds, subsubCategoryIds }) {
  console.log('Original funding audience data:', { identityIds, categoryIds, subcategoryIds, subsubCategoryIds });

  // Validate and fix hierarchy
  const validated = await validateAudienceHierarchy({ categoryIds, subcategoryIds, subsubCategoryIds });

  console.log('Validated funding audience data:', validated);

  // Properly clear existing associations and set new ones
  if (identityIds !== undefined) {
    await funding.setAudienceIdentities([]); // Clear first
    if (identityIds && identityIds.length > 0) {
      await funding.setAudienceIdentities(identityIds);
    }
  }

  if (categoryIds !== undefined) {
    await funding.setAudienceCategories([]); // Clear first
    if (validated.categoryIds && validated.categoryIds.length > 0) {
      await funding.setAudienceCategories(validated.categoryIds);
    }
  }

  if (subcategoryIds !== undefined) {
    await funding.setAudienceSubcategories([]); // Clear first
    if (validated.subcategoryIds && validated.subcategoryIds.length > 0) {
      await funding.setAudienceSubcategories(validated.subcategoryIds);
    }
  }

  if (subsubCategoryIds !== undefined) {
    await funding.setAudienceSubsubs([]); // Clear first
    if (validated.subsubCategoryIds && validated.subsubCategoryIds.length > 0) {
      await funding.setAudienceSubsubs(validated.subsubCategoryIds);
    }
  }
}
