...
const [
  ...
  UserCategory.findAll({
    where: { 
      userId: { [Op.in]: userIds },
     // ...(Object.keys(interestsWhere).length ? interestsWhere : {})  //warning (it makes user.userSubcategories not be shown but commenting may break other things!)
    }, 
    include: [
      { model: Category, as: "category", required: false },
      { model: Subcategory, as: "subcategory", required: false },
...
// Attach associations to each user object AND apply filtering
 ...
 console.log({effAudienceSubcategoryIds,effAudienceCategoryIds,effAudienceCategoryIds })
...
const filteredRows = rows.filter(user => {
  ...
  user.industryCategories = associations.industryCategories || [];
   ...
   if(user.id=="c0fac4bf-14b6-4171-a882-5a4ebdb7df5f"){
  ...
  if(user.id=="c0fac4bf-14b6-4171-a882-5a4ebdb7df5f"){
  console.log({effAudienceSubcategoryIds,associations:associations.userSubcategories.map(i => String(i.subcategoryId))})
  }
